...
arma::mat SB_compute_tau (arma::mat V, arma::mat B, arma::rowvec pi, arma::mat Y, int k, int n) {
  ...
  for (int i = 0; i < n; i++) {
    ...
    for (int q = 0; q < k; q++) {
      ...
      V_new(i, q) = arma::accu(BY) + log(pi(q));
    }
  }
...
Rcpp::List SB_VE_step (arma::mat V, arma::mat B, arma::rowvec pi, arma::mat Y, int k, int n, double tol_lk, int maxit_FP) {
  ...
  while (!stop) {
    it += 1;
    V_new = SB_compute_tau(V, B, pi, Y, k, n);
...
Rcpp::List SB_VEM (arma::mat Y, int k, int n, double tol_lk, int maxit, int maxit_FP, arma::mat V) {
  ...
  while (!stop) {
    ...
    pi = SB_compute_pi(V, n);
    
    // 2. VE-Step
    Rcpp::List VE = SB_VE_step(V, B, pi, Y, k, n, tol_lk, maxit_FP);
    arma::mat V_aux = VE["V"];
    V_new = V_aux;
    int it_FP = VE["it"];
    
    // 3. Compute ELBO
    J = SB_compute_ELBO(V_new, B, pi, Y, k, n);
    J_vec(it-1) = J;
  ...
  return Rcpp::List::create(Rcpp::Named("LogLik") = LogLik,
                            ...
                            Rcpp::Named("B") = B,
                            Rcpp::Named("k") = k, 
                            Rcpp::Named("np") = n_pi_par + n_B_par,
                            Rcpp::Named("V") = V, 
                            Rcpp::Named("icl") = ICL);
}
...
Rcpp::List SB_LastME_step (arma::mat Y, int n, arma::mat V, int k, int R, int maxit_FP, double tol_lk, double tol_theta) {
  ...
  J = SB_compute_ELBO(V, B, pi, Y, k, n);
  double LogLik = SB_compute_LogLik(V, B, pi, Y, k, n);
  int n_B_par = bin_coeff(k+2-1, 2);
  int n_pi_par = k-1;
  double ICL_B_par = 0.5 * n_B_par * log(bin_coeff(n, 2));
